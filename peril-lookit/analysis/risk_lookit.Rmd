...
ggplot(data = risk.long %>% filter(cost=="Risk"), aes(as.factor(trial_n), look, fill=trial_type)) + 
  geom_boxplot() +
  stat_summary(fun.y=mean,geom="point",shape=5)+
  stat_summary(fun.data =mean_cl_boot, geom="errorbar",width=0.1)+
...
```{r}
(habplot7 | habplot6) / (habplot8 | habplot9) + plot_annotation(tag_levels = 'A')
...
habplot6 <- ggplot(data=risk.wide, aes(sum_fam, avg.higher-avg.lower))
habplot6 + geom_point() +
  facet_wrap(~experiment)+
  geom_smooth(method = "lm")+
  ylab("<-- Pref for expected ---- Pref for unexpected -->")+
  xlab("Total Attention During Habituation (s)")
```
