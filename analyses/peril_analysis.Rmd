...
#### Comparing 10mo in Exp 4, Study 1 to 10mo in Liu et al. (2017), Study 3   
```{r}

widthvsdepth.10mo <- long.avg %>% 
  filter(exp == "Exp4.Study1" | exp == "LUTS.Exp.3") %>%
  mutate(trench_manipulation = case_when(cost == "Gap" ~ "width",
                                         cost == "Danger" ~ "depth"))

widthvsdepth.10mo$type <- relevel(widthvsdepth.10mo$type, ref = "higher")


widthvsdepth.10mo.1 <- lmer(loglook ~ type * trench_manipulation + (1|subj),
               data = widthvsdepth.10mo)

# id influential observations
plot(influence(widthvsdepth.10mo.1, "subj"), which = "cook",
     cutoff = 4/64, sort = TRUE,
     xlab = "Cook´s Distance",
     ylab = "Subject ID")
# one influential observation, exclude

widthvsdepth.10mo.1.cooks <- lmer(loglook ~ type * trench_manipulation + (1|subj),
               data = widthvsdepth.10mo %>% filter(subj != "S3_15"))

widthvsdepth.10mo.1.table <- gen.m(widthvsdepth.10mo.1.cooks)
widthvsdepth.10mo.1.ci <- gen.ci(widthvsdepth.10mo.1.cooks)[3:6,]

widthvsdepth.10mo.1.beta <- lmer(scale(loglook) ~ type * trench_manipulation + (1|subj),
               data = widthvsdepth.10mo %>% filter(subj != "S5_25"))
widthvsdepth.10mo.1.betas <- gen.beta(widthvsdepth.10mo.1.beta)

widthvsdepth.10mo.results <- cbind(widthvsdepth.10mo.1.table, widthvsdepth.10mo.1.betas,widthvsdepth.10mo.1.ci)

widthvsdepth.10mo.cohensd <- lme.dscore(widthvsdepth.10mo.1.cooks,
           data = filter(widthvsdepth.10mo, subj != "S3_15"),
           type = "lme4") %>% select(d) %>% slice(3) %>% as.numeric() 
```
...
Our final sample included a grand total of 102 10-month-old infants. Studies 1-2 were conducted in the lab, and our final sample included `r info(exp4.study1.avg)$n` infants in Study 1 (M = `r info(exp4.study1.avg)$mean` months, range = `r info(exp4.study1.avg)$min`-`r info(exp4.study1.avg)$max`, `r info(exp4.study1.avg)$f` female; an additional 6 infants tested and excluded from the final sample), and `r info(exp4.study2.avg)$n` infants in Study 2 (M = `r info(exp4.study2.avg)$mean` months, range = `r info(exp4.study2.avg)$min`-`r info(exp4.study2.avg)$max`, `r info(exp4.study2.avg)$f` female; an additional 2 infants tested and excluded). In Study 3, we collected an online sample of `r info(exp4.study3.avg)$n` (M = `r info(exp4.study3.avg)$mean` months, range = `r info(exp4.study3.avg)$min`-`r info(exp4.study3.avg)$max`, `r info(exp4.study3.avg)$f` female, xxxx excluded). In the online sample, infants sat in a high chair (`r exp3b.highchair %>% filter(highchair == "1") %>% select(n) %>% as.numeric()` out of 40 participants) or their caregivers’ laps (`r exp3b.highchair %>% filter(highchair == "0") %>% select(n) %>% as.numeric()`/40), depending on caregiver preferences, and watched the displays on a tablet (`r exp3b.devices %>% filter(device == "ipad") %>% select(n) %>% as.numeric()`/40) or a laptop computer (`r exp3b.devices %>% filter(device == "laptop") %>% select(n) %>% as.numeric()`/40). Caregivers gave high ratings for both the video quality (M = `r exp3b.qualratings$vquality`, SD = `r exp3b.qualratings$vquality.sd`) and audio quality (M = `r exp3b.qualratings$aquality`, SD = `r exp3b.qualratings$aquality.sd`). All three of these studies were pre-registered (Study 1: https://osf.io/uh8ns/; Study 2: https://osf.io/kx928/, Study 3: https://osf.io/48j9v/)
...
When we tested 10-month-old infants using identical protocols as reported in Experiment 1, these younger infants did not show a statistically significant looking preference between the test events, (Mhighervalue = `r summary.avg %>% filter(exp == "Exp4.Study1") %>% filter(type == "higher") %>% select(look) %>% as.numeric()`s, Mlowervalue =  `r summary.avg %>% filter(exp == "Exp4.Study1") %>% filter(type == "lower") %>% select(look) %>% as.numeric()`, pooled SE = `r summary.avg %>% filter(exp == "Exp4.Study1") %>% filter(type == "lower") %>% select(se) %>% as.numeric()`, `r report(exp1b.results,2,3,1)` d = `r exp1b.cohensd`, removing 1 influential participant). Comparing the data from Experiment 1 and Experiment 4, Study 1, 10- and 13-month-old infants did not significantly differ in their looking preferences in this task, `r report(exp1.1013.results,4,3,2)`, d = `r exp1.1013.cohensd`, removing 1 influential participant. Comparing 10-month-old infants' responses in this study, that manipulates trench depth, with the responses of infants at the same age to an analogous study, that manipulates trench width, infants' responses to the test events were not different depending on whether the task manipulated depth vs width `r report(widthvsdepth.10mo.results,4,3,2)`, d = `r widthvsdepth.10mo.cohensd`, removing 1 influential participant.

#### Avoiding danger (Studies 2-3) 
When we tested 10-month-old infants in identical protocols as Experiment 2, ten-month-old looked longer at test when the agent chose the deeper the shallower trench (Mdeeper = `r summary.avg %>% filter(exp == "Exp4.Study2") %>% filter(type == "higher") %>% select(look) %>% as.numeric()`s, Mshallower =  `r summary.avg %>% filter(exp == "Exp4.Study2") %>% filter(type == "lower") %>% select(look) %>% as.numeric()`, pooled SE = `r summary.avg %>% filter(exp == "Exp4.Study2") %>% filter(type == "lower") %>% select(se) %>% as.numeric()`, `r report(exp2b.1.results,2,3,1)` d = `r exp2b.test.cohensd*-1`, removing 1 influential participant). During control events, 10-month-old infants did not show a significant looking preference (Mdeeper = `r summary.avg %>% filter(exp == "Exp4.Study2") %>% filter(type == "deep") %>% select(look) %>% as.numeric()`s, Mshallower =  `r summary.avg %>% filter(exp == "Exp4.Study2") %>% filter(type == "shallow") %>% select(look) %>% as.numeric()`, pooled SE = `r summary.avg %>% filter(exp == "Exp4.Study2") %>% filter(type == "deep") %>% select(se) %>% as.numeric()`, `r report(exp2b.results2,2,3,2)` d = `r exp2b.control.cohensd*-1`, excluding 1 influential participant). In contrast to the data from older infants, these two patterns of looking preference did not differ from each other, `r report(exp2b.results3,4,3,2)`, d = `r exp3b.interaction.cohensd`no influential participants.

Notably, we did not replicate the results of Study 2 when we ran an additional online sample of infants: In Study 3, 10-month-old infants did not show a looking preference during the test events (Mdeeper = `r summary.avg %>% filter(exp == "Exp4.Study3") %>% filter(type == "higher") %>% select(look) %>% as.numeric()`s, Mshallower =  `r summary.avg %>% filter(exp == "Exp4.Study3") %>% filter(type == "lower") %>% select(look) %>% as.numeric()`, pooled SE = `r summary.avg %>% filter(exp == "Exp4.Study3") %>% filter(type == "lower") %>% select(se) %>% as.numeric()`, `r report(exp3b.1.results,2,3,1)` d = `r exp3b.test.cohensd`, excluding 1 influential participant), or the control events (Mdeeper = `r summary.avg %>% filter(exp == "Exp4.Study3") %>% filter(type == "deep") %>% select(look) %>% as.numeric()`s, Mshallower =  `r summary.avg %>% filter(exp == "Exp4.Study3") %>% filter(type == "shallow") %>% select(look) %>% as.numeric()`, pooled SE = `r summary.avg %>% filter(exp == "Exp4.Study3") %>% filter(type == "deep") %>% select(se) %>% as.numeric()`, `r report(exp3b.3.results,2,3,2)` d = `r exp3b.control.cohensd*-1`, no influential participants), and their looking preferences did not differ across the two phases of the experiment, `r report(exp3b.2.results,4,2,2)`, d = `r exp3b.interaction.cohensd`, no influential observations.
...
### Experiment 1
Infants looked longer at test when the agent, at test, chose the deeper trench over the shallower trench (`r report2(exp1.everyone.test.results, 2, 2, 2)`).  These findings accord with those reported in the main text and support the interpretation that infants expected the agent to choose the goal for which it was willing to jump deeper trenches.
...
exp1.fam.bymovie.wide <- exp1.fam.bymovie %>%
  pivot_wider(names_from = videoclip, values_from = proportion.on.total, id_cols = c(subj))

exp1.avg.diff <- exp1.avg %>%
  filter(phase == "testavg") %>%
  pivot_wider(names_from = type, values_from = look, id_cols = subj) %>%
  mutate(delta.look = lower-higher)

exp1.fam.glancedoff.wide <- exp1.fam.glanced.off.summary %>%
  pivot_wider(names_from = videoclip, values_from = prop.glancedoff, id_cols = subj)
...
# Plots from Experiment 4

```{r fig.exp4/study1, include = TRUE, fig.width = 4}

theme_set(theme_cowplot(font_size = 20))

exp4.study1.fig.data <- exp4.study1.avg
levels(exp4.study1.fig.data$type) <- c(NA, "higher", "lower", NA)
exp4.study1.fig.data$type <- relevel(exp4.study1.fig.data$type, ref = "higher")
colors1 <- c(wes_palettes$Zissou1[3], wes_palettes$Zissou1[2])

exp4.fig <- ggplot(data = exp4.study1.fig.data %>% filter(!is.na(type)), aes(type, look, fill = type)) +
  geom_boxplot() +
  scale_fill_manual(values = colors1) +
    geom_errorbar(data = summary.avg %>% filter(exp == "Exp4.Study1"), colour = "red", position = position_dodge(width = 5), width = 0, aes(ymin = look-ci, ymax = look+ci)) +
  stat_summary(fun = mean, alpha = 0.8, geom = "point", shape = 21, size = 3, position = "dodge", colour = "red", fill = "red") +
  ylab("Looking Time (s)") +
  xlab("Test event") +
  coord_cartesian(ylim = c(0, 65)) +
...
**Figure S5.** Looking time towards test events in Experiment 4 Study 1. 

```{r fig.exp4.study23, include = TRUE, fig.width = 8, fig.height = 8}

exp4.study23.figure <- rbind(exp4.studies23.avg) %>%
  mutate(cliff = case_when(type == "deep" | type == "higher" ~ "deep",
                           type == "shallow" | type == "lower" ~ "shallow"))
exp4.study23.figure$cliff <- as.factor(exp4.study23.figure$cliff)
exp4.study23.figure$cliff <- relevel(exp4.study23.figure$cliff, ref = "shallow")
exp4.study23.figure$phase <- as.factor(exp4.study23.figure$phase)

levels(exp4.study23.figure$cliff)
levels(exp4.study23.figure$phase) <- c("control", "test")
exp23.colors <- c(wes_palette("Royal2")[2], wes_palette("Royal2")[1])

exp4.study2.fig <- ggplot(data = exp4.study23.figure %>% filter(exp == "Exp4.Study2"), aes(cliff, look, fill = cliff)) +
  geom_boxplot(aes(alpha = phase)) +
  stat_summary(fun = mean, alpha = 0.8, geom = "point", shape = 21, size = 3, position = "dodge", colour = "red", fill = "red") +
  geom_errorbar(data = summary.avg %>% filter(exp == "Exp4.Study2"), colour = "red", position = position_dodge(width = 5), width = 0, aes(ymin = look-ci, ymax = look+ci)) +
  ylab("Looking Time (s)") +
  xlab("Cliff Depth") +
  coord_cartesian(ylim = c(0, 65)) +
  geom_point(alpha = 0.1) +
  geom_line(alpha = 0.2, aes(group = subj)) +
  facet_wrap(exp~phase, nrow = 1) +
  theme(legend.position = "none") +
  scale_fill_manual(values = exp23.colors) +
  scale_alpha_discrete(range = c(0.4, 1)) +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) 

exp4.study3.fig <- ggplot(data = exp4.study23.figure %>% filter(exp == "Exp4.Study3"), aes(cliff, look, fill = cliff)) +
  geom_boxplot(aes(alpha = phase)) +
  stat_summary(fun = mean, alpha = 0.8, geom = "point", shape = 21, size = 3, position = "dodge", colour = "red", fill = "red") +
  geom_errorbar(data = summary.avg %>% filter(exp == "Exp4.Study3"), colour = "red", position = position_dodge(width = 5), width = 0, aes(ymin = look-ci, ymax = look+ci)) +
  ylab("Looking Time (s)") +
  xlab("Cliff Depth") +
  coord_cartesian(ylim = c(0, 65)) +
  geom_point(alpha = 0.1) +
...
exp4.studies233.fig <- ggplot(data = exp4.study23.figure %>% filter(exp == "Exp4.Study3" | exp == "Exp4.Study2"), aes(cliff, look, fill = cliff)) +
  geom_boxplot(aes(alpha = phase)) +
  stat_summary(fun = mean, alpha = 0.8, geom = "point", shape = 21, size = 3, position = "dodge", colour = "red", fill = "red") +
  geom_errorbar(data = summary.avg %>% filter(exp == "Exp4.Study3" | exp == "Exp4.Study2"), colour = "red", position = position_dodge(width = 5), width = 0, aes(ymin = look-ci, ymax = look+ci)) +
  ylab("Looking Time (s)") +
  xlab("Cliff Depth") +
  coord_cartesian(ylim = c(0, 65)) +
  geom_point(alpha = 0.1) +
  geom_line(alpha = 0.2, aes(group = subj)) +
  facet_wrap(~exp+agegroup+phase, nrow = 1) +
  theme(legend.position = "none") +
  scale_fill_manual(values = exp23.colors) +
  scale_alpha_discrete(range = c(0.4, 1)) +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) 

exp4.study2.fig + exp4.study3.fig + plot_annotation(tag_levels = 'A')
