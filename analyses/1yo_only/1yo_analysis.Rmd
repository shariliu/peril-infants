...
Our final sample included a grand total of 102 10-month-old infants. Studies 1-2 were conducted in the lab, and our final sample included `r info(exp1b.avg)$n` infants in Study 1 (M=`r info(exp1b.avg)$mean` months, range=`r info(exp1b.avg)$min`-`r info(exp1b.avg)$max`, `r info(exp1b.avg)$f` female; an additional 6 infants tested and excluded from the final sample), and `r info(exp2b.avg)$n` infants in Study 2 (M=`r info(exp2b.avg)$mean` months, range=`r info(exp2b.avg)$min`-`r info(exp2b.avg)$max`, `r info(exp2b.avg)$f` female; an additional 2 infants tested and excluded). In Study 3, we collected an online sample of `r info(exp3b.avg)$n` (M=(M=`r info(exp3b.avg)$mean` months, range=`r info(exp3b.avg)$min`-`r info(exp3b.avg)$max`, `r info(exp3b.avg)$f` female, xxxx excluded). In the online sample, infants sat in a high chair (`r exp3b.highchair %>% filter(highchair=="1") %>% select(n) %>% as.numeric()` out of 40 participants) or their caregivers’ laps (`r exp3b.highchair %>% filter(highchair=="0") %>% select(n) %>% as.numeric()`/40), depending on caregiver preferences, and watched the displays on a tablet (`r exp3b.devices %>% filter(device=="ipad") %>% select(n) %>% as.numeric()`/40) or a laptop computer (`r exp3b.devices %>% filter(device=="laptop") %>% select(n) %>% as.numeric()`/40). Caregivers gave high ratings for both the video quality (M=`r exp3b.qualratings$vquality`, SD=`r exp3b.qualratings$vquality.sd`) and audio quality (M=`r exp3b.qualratings$aquality`, SD=`r exp3b.qualratings$aquality.sd`). All three of these studies were pre-registered (Study 1: https://osf.io/uh8ns/; Study 2: https://osf.io/kx928/, Study 3: https://osf.io/48j9v/)
	Data reliability. As in Experiments 1-3, the reliability of the looking time data in Experiment 4 were high (Study 1: ICC=0.995, 95% CI [0.991, 0.997]; Study 2: ______; Study 3:______) 
