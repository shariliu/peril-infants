...
exp1.fam.bymovie %>%
  ggplot(aes(yesno, total.lt, fill=depth)) + 
  geom_boxplot() +
  geom_point() +
  # geom_line(alpha = 0.2, aes(group = subjID)) +
  ylab("Looking time (s)") +
  xlab("Accept (yes) or Refuse (no)") +
  facet_wrap(~depth) + 
  stat_summary(fun.y = mean, geom = "point", shape=3, size=4, position = "dodge", colour = "black")
```

```{r}

fam1 <- lmer(data = exp1.fam.bymovie,
               formula = total.lt ~ depth + (1|subj))
summary(fam1)
...
exp1.fam.bymovie.wide <- exp1.fam.bymovie %>%
  pivot_wider(names_from = videoclip, values_from = total.lt, id=subj)
...
fam2 <- lm(data = exp1.famtest,
             formula = delta.look ~ shallow_yes + medium_no + medium_yes + deep_no)
